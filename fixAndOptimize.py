...
def evaluate(x, timeout):
    ...
    path_to_instance = r"encoding.dimacs"
    wcnf = WCNF(path_to_instance)
    for v in x:
        wcnf.append([v])

    path_to_formula = r"C:\Projects\Honours\FixAndOptimizeTimetabling\current_formulab1.wcnf"
    ...
    # " | findstr /R /C:\"v..\(.*\)\"" +
    print("era lenx")
    command = r"pumpkin-cli.exe -t " + str(timeout) + " " + path_to_formula + " > " + r"capture_sol.txt"
...
while k <= len(arr):
    ...
    with open("solution_memory.txt", 'w') as f:
        for item in curr_sol:
            f.write(str(item) + ' ')
    curr_cost = evaluate(curr_sol, 60000)
    print(cost, " the current cost is: ", curr_cost)
    if curr_cost < cost:
        cost = curr_cost
        ind = 0
